...
if __name__ == "__main__":
    ...
    # example from T&K (1992)
    PROB = 0.16666666667
    print(cpt(utilFunc=lambda x: x,
              weightFuncGains=lambda p: p,
              weightFuncLosses=lambda p: p,
              refOutcome=0,
              outcomes=[-5.0, -1.0, 6.0, -3.0, 4.0, 2.0],
              probabilities=6*[PROB]))
    # CptResult(ExpectedValue=0.50000000001,
    #           CertaintyEquiv=np.float64(0.50000000001))

    # example from Aleks
    print(cpt(utilFunc=lambda x: utilPower(x, alpha=0.88,
                                           beta=0.88, lamb=2.25),
    ...
    # CptResult(ExpectedValue=0.5142746670673448,
    #           CertaintyEquiv=np.float64(4.733729209057415))

    # example with no probs, non-unique float outcomnes, and
    # non-zero ref point
    print(cpt(utilFunc=lambda x: utilPower(x, alpha=0.88,
                                           beta=0.88, lamb=2.25),
    ...
    # CptResult(ExpectedValue=0.22542186628262106,
    #           CertaintyEquiv=np.float64(12.135541394014753))

    # example using recommended functional forms and parameters from
    # Bouchoiuicha & Vieider (2017).
    print(cpt(utilFunc=lambda x: utilNormLog(x, gamma=1.223, delta=0.0,
                                             lamb=2.25),
              weightFuncGains=lambda p: weightPrelec2(p, alpha=0.53,
                                                      beta=0.969),
              weightFuncLosses=lambda p: weightPrelec2(p, alpha=0.623,
    ...
    # CptResult(ExpectedValue=-22352.784861502496,
    #           CertaintyEquiv=np.float64(110125.74923511136))
    ...
    print(cpt(utilFunc=lambda x: utilPower(x, alpha=0.88,
              ...
              outcomes=[70000.0, 80000.0, 90000.0, 100000.0,
                        110000.0, 120000.0, 130000.0]))
    # CptResult(ExpectedValue=-3563.361982439633,
    #           CertaintyEquiv=np.float64(103982.99776749458))
    ...
    print(cpt(utilFunc=lambda x: utilPower(x, alpha=0.88,
              ...
              outcomes=[70.0, 80.0, 90.0, 100.0,
                        110.0, 120.0, 130.0]))
    # CptResult(ExpectedValue=-8.163190700673397,
    #           CertaintyEquiv=np.float64(103.9829977674946))
